...
try:
    nlp = spacy.load("en_core_web_sm")  # Using standard English model
except OSError:
...
class PDFProcessor:
    ...
    async def process_pdf(self, file_path: str) -> ProcessingResult:
        ...
        try:
            ...
            if await self.supabase.check_document_exists(file_hash):
    ...
    async def store_document(self, document: ExtractedDocument) -> bool:
        ...
        try:
            # Convert document to dict and handle deadlines separately
            doc_data = document.dict()
            deadlines = doc_data.pop('deadlines')
            
            # Store document and deadlines
            return await self.supabase.store_scheduling_order(doc_data, [d.dict() for d in deadlines])
        except Exception as e:
